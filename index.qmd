...
Every step is independent from the other : one will be able to start from the next step if a step is too difficult.
...
In addition, some transaction are registered in the data when they are of little interest to us. For example:
...
| 1200 ≤ prop_year ≤ 2022 | Pas de modification | 1200 → 1200 |
...
Create a VScode-Python service on SSP Cloud. In the service, open a terminal by clicking on {{< fa brands fa-bars >}}, then `Terminal > New Terminal`. Clone the project repository with the following command after replacing `OWNER_NAME` with your name on Github from the previous step:
