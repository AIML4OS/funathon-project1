...
y_train_df["quantile"] = pd.qcut(y_train_df["price_sqm_log"], q=100, labels=False) ## allows to discretly cut along quantiles
y_sub = y_train_df.groupby("quantile").sample(frac=0.1, random_state= RANDOM_STATE)  # sampling by quantile 

y_sub = y_sub["price_sqm_log"] # converting to pandas.series
...
grid_search = GridSearchCV(
    estimator=model, # it is the model created in the preprocessing part
