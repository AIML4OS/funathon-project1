...
1.  Using the `RandomForestRegressor` class and its [scikit-learn documentation page](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestRegressor.html), define a basic model with the following hyperparameters :
    ...
    As it's only a first step we chose to simplify the exercise. We will drop the `trans_date` and `prop_type` features. 

    We will come back to this later on and chain it with the preprocessing steps.

    Train the model (it should take a few minutes to train).
...
::: callout-note
For convenience and pedagogical reasons, we are still working on a subset of the features. In the real world, we would do this step with the pipeline defined at the end of the [pre-processing part](1-preprocessing.qmd).
:::
...
0.  For convenience, we dropped the features `prop_type`, `dtemut`. Get them back and define the new training and testing datasets `X_train, X_test, y_train, y_test`.
...
1.  Create a dictionary with the 3 hyperparameters to test (`n_estimators`, `max_features` and `min_samples_leaf`) for training.
    ...
    Pay attention to hyperparameter names : when using `Pipeline` objects, theses names are more complex than plain `RandomForestRegressor` parameters names. In the previous section, we used two objects for training :
    ...
    -   `Pipeline` to apply all preprocessing transformations and fit the model : you need to refer to the name of the random forest step (it was `'RF'`)
        For example, for the hyperparameter `n_estimators`, the key in the parameter dictionary must be `RF__n_estimators`
...
predicted_price_sqm = np.exp(rf_model_final.predict(X_examples[feature_cols]))
